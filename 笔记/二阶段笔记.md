...
10. 数据类型
    ...
    + 声明变量并初始化为undefined
      ...
      + 对象
        ...
        3. 对象中不能出现重复的属性，定义重复的属性不会报错，但是后定义的会覆盖前面定义的属性

      1. typeof操作符用来检测变量的数据类型，是个操作符，不是函数

  11. JavaScript不支持任何创建自定义类型的 机制
...
1. 对象基础
   ...
   ```
    ...
    + 点语法只能获取属性名明确的字符串属性
    + **对象都会转换为[Object Object]，所以会覆盖前面的，但是数组不会覆盖**
   ```
   var d = "add";
   var obj = {
   	name: "李岚",
...
4. 删除对象中的属性
   ...
   + delete 不能删除window下的属性和方法，可以删除值，但是不能删除属性
   
   + **删除未使用var定义的变量后。比如delete b，window.b和b就不是一样的了**
   
     ```
     b=20;
     console.log(a);//10
     delete b;//删除了window.b的值，但是没有删除window.b的定义
...
2. 案例
   ...
   + [斐波那契数列](./案例/递归-斐波那契数列.html)
   + [求阶乘](./案例/递归求100的阶乘.html)
   + [求两个数字的最大公约数](./案例/递归求两个数字的最大公约数.html)
...
1. 数组的元素简称元，访问方式arr[index]，这种方式叫做下标变量
...
3. 创建数组的方式
   ...
   + 构造函数创建数组```var arr = new Array(1,2,3)```
     ...
     缺点：构造函数创建数组时只有一个自然数参数，则这个参数代表数组的长度，数组为空，如果是负数或者小数，就会报错。如果是非数值，就会作为数组中的第一个元素
...
21. **reduce()**，将数组元素计算为一个值，从左向右，使用返回，value和item必需，底层是用Number对其他非数值类型数组元素进行转换：因为null相加转换为了0，undefined转换为了NaN，**相当于归并**
...
9. **slice()**提取字符串，并在新的字符串中返回被提取的部分，只能从前向后选
...
12. **substring()**提取字符串中两个指定的索引号之间的字符，不支持负数，负数代表最前面，只会从后向前选，start>end即可，注：不是节选的字符串颠倒
...
1. open()
   ...
   功能：新建浏览器标签页，打开网页，如果设置了打开的宽高等样式，就会新建浏览器窗口进行打开

   参数：要打开的网页，网页名字、网页的样式“width=500”

   ```
   open("http://www.baidu.com","百度","width:500px")
   ```
...
4. outerWidth、outerHeight
   ...
   宽度比上面多了滚动条等，高度比上面多了工具栏等

5. screenLeft、screenTop

   属性：获取当前网页在整个电脑屏幕的位置，左上角为坐标原点
...
1. location.reload()
   ...
   直接在页面中这样写的话，会陷入刷新的死循环

2. location.href = "http://www.baidu.com"

   console.log(location.href)

   属性：跳转打开页面，浏览器有历史记录，作为属性，可以改变和获取，可以获取当前页面的地址
   ...
   设置当前页面的地址，如果当前页面的地址和设置的不一样就相当于跳转
...
5. location.hash
   ...
   属性：获取到url中#后的字符串，如果没有，则返回空字符串。获取锚点
...
9. location.port
   ...
   属性：返回 web 主机的端口 （80 或 443）

10. location.protocal

    属性：返回所使用的 web 协议（http:// 或 https://）
...
1. screen.availWidth、screen.availHeight
...
1. navigator.userAgent
   ...
   属性：返回由客户机发送服务器的 user-agent 头部的值
...
1. node：标签、注释、文本（回车）、文档

2. 节点属性

   + 节点名称nodeName

     1. 元素节点的 nodeName 是大写的标签名称 
     ...
     ```
     console.log(document.body.firstChild.constructor=="DIV")
     console.log(document.body.firstElementChild.constructor===HTMLDivElement);
     ```
...
1. document.getElementById()
   ...
   ```
   var elem = document.getElementById("div0");
   console.log(elem.firstElementChild);//可以
   ```

2. document.getElementsByTagName()

   功能：通过标签名获取标签列表（HTMLCollection，类似数组），只能通过document获取
...
3. document.getElementsByClassName(新)

   功能：通过class名获取标签列表，标签有多个class时不影响获取，任何标签都可以通过该函数获取其子项的符合条件的列表

4. document.getElementsByName()

   功能：获取表单元素到NodeList

   + 页面中除了HTML标签，还有XHTML标签，XHTML标签不属于HTMLCollection，但是属于node，该函数可以在XHTML使用
   + 非表单元素不行，可以获取到，不报错，但是没意义，因为name不是其他标签的原生属性，而且不能提交
   + 只能通过document获取
   + NodeList自身有一些方法可供使用（forEach遍历，等等），HTMLCollection没有

5. document.querySelector( 新)  

   功能：所有选择器，得到选择器列表中的第一个元素

   + 任何标签都可以通过该函数获取其子项的符合条件的列表
...
1. appendChild(node)
   ...
   功能：向childNodes末尾插入一个节点node

2. 元素.insertBefore(newElement,targetElement)

   功能：向元素中targetNode之前插入节点node

   参数：要插入的元素，插入在谁之前

【注】将一个节点放在页面中就会重新打开DOM树，重新渲染加载，因此在写页面的时候一定要注意，尽量减少打开DOM树的次数
...
【注】删除时，元素仅仅是从页面上删除，不是从内存中删除，在没有清除内存的情况下，还可以加回来，从页面中删掉，但是事件仍然保存，即使设置为null也不可以，需要在设置null之前将事件也删除
...
1. cloneNode(boolean)
   ...
   参数：布尔值，指的是深复制（true）还是浅复制（false）

   ​			深复制：复制元素和元素的所有子元素和节点

   ​			浅复制：仅复制元素的属性和该节点

   【注】复制标签的时候，会将标签的属性一起复制，有ID的标签，在复制的时候要将ID修改
...
1. DOM的对象属性，分为自定义型和原DOM对象属性

2. DOM对象的原属性和DOM对象的标签属性部分对应，部分有区别
...
1. 元素.setAttribute(属性,值)

   功能：设置DOM标签属性

2. 元素.getAttribute(属性)

   功能：获取DOM标签属性

3. 元素.removeAttribute(属性)

   功能：删除DOM标签属性

4. DOM标签属性命名，不能使用大小写和下划线区分，必须全小写字母，使用横杠区分每个单词

**案例 [DOM打印表格](./案例/DOM打印表格.html)    [找徐峥](./案例/找徐峥.html)**
...
1. 设置Dom对象的样式
   ...
   ```
   这种写法需要将所有的CSS中"-"字母，替换为字母大写
   dom.style.styleName=""
   
   这种写法按照CSS样式书写
   dom.style = "width:100px;height:100px;background-color:red;"
   
   var div = document.querySelector("#div1") 
   div.style.width="100px"//CSS样式没有提示
   div.style.backgroundColor="#000"
   
   var div:HTMLDivElement = document.querySelector("#div1") 
   div.style.width="100px"//CSS样式有提示,但是报错，写完CSS之后再删掉HTMLDivElement
   
   function setDivStyle(elem:HTMLDivElement)
   
   原因：JavaScript不能识别变量的类型
   ```

   【注】Object.assign(obj1,obj)，将obj复制给obj1，浅复制，之间无引用关系

   ```
   利用Object.assign给Dom设置样式
   ...
   div.className = div.className.replace("div1","")
   ```
...
2. 获取计算后的dom样式
   ...
   + 如果是行内样式，可以通过下面的方式获取，如果样式在CSS中，下面这种方式不能获取

     ```
     div.style.width
     ```

   + getComputedStyle(dom)，获取计算后的样式
   ...
   try{
   	...
   	style = getComputedStyle(div).width;
   }
   ```

   + styleSheets，获取元素的CSS样式 
...
[JavaScript距离详解](https://blog.csdn.net/qq_39207948/article/details/87894583?utm_medium=distribute.pc_relevant.none-task-blog-BlogCommendFromBaidu-2.nonecase&depth_1-utm_source=distribute.pc_relevant.none-task-blog-BlogCommendFromBaidu-2.nonecase)

#### 宽高

##### DOM的宽高问题

+ clientWidth    clientHeight	计算后宽高 = 宽高 + padding - 滚动条宽高（如果有的话）
+ offsetWidth    offsetHeight    计算后宽高 = 宽高 + padding + border
+ scrollWidth    scrollHeight    
  + 如果没有超出，等同于clientWidth    clientHeight
  + 没有滚动条的时候，如果超出了，就是实际所占的宽高，（从左边的border的右侧到超出内容结束）
  + 有滚动条，如果超出了，第二情况减去滚动条宽高

##### html和body宽高问题

1. body外面自带8px的margin
2. body不会因为高度撑开而变化，但是会因为滚动条产生而变化

+ clientWidth    clientHeight    当前文档的可视宽高 - 滚动条
+ offsetWidth    offsetHeight    当前文档的内容宽高 - 滚动条**（页面中啥也没有的时候，高度是8px，因为body有默认margin）**
+ scrollWidth    scrollHeight    当前文档的可视宽高，如果有滚动条，就是实际body撑开的高度
...
//当前文档的可视宽高-(如果有滚动条，减去滚动条宽高)           console.log(document.documentElement.clientWidth,document.documentElement.clientHeight);
...
1. 事件的运行过程
   ...
   先创建一个侦听的对象，侦听到的时候就执行对应的函数

   创建一个事件，将事件抛给侦听的对象，先侦听后抛发

2. 事件的类型必须相同，事件侦听的对象必须相同，然后将事件抛向侦听

3. 部分事件是系统默认事件，这些事件会由系统自动抛发
...
5. e.target = 点击谁就是谁，e.current = this (侦听的对象，侦听谁就是谁)
...
**案例 [七彩色方块](./案例/七彩色方块.html)  [轮播图1](./案例/轮播图.html)  [轮播图2](./案例/轮播图的切换.html)  [城市列表](./案例/城市列表.html)**

### 删除事件

1. 移除事件：element.removeEventListener("事件类型"，侦听时执行的回调函数)
...
+ 属性
  ...
  1. clientX、clientY
     ...
     + 距离可视区域左上角，跟页面是否滚动没有关系，只是跟可视窗口的左上角的位置

  2. offsetX、offsetY

     + 鼠标相对事件目标对象（e.target）的左上角的距离

  3. layerX、layerY

     + 如果目标对象是定位方式，值与offset相似

     + 如果目标对象是非定位形式，取父容器的左上角的位置，如果父容器未定位，继续向上寻找已定位的元素
  ...
  9. 针对mousedown事件的属性，当按下左键的时候（001），中键（142），右键（223）
...
+ 事件（鼠标事件的顺序：按下、松开、点击）
  ...
  + 具有冒泡的功能，可以监听子元素，可以收到鼠标划入滑出子元素的冒泡事件
...
+ 当使用contextmenu事件的时候，取消默认事件可以取消鼠标右键菜单
+ 当针对元素拖拽的时候，元素内有文本，文本会被选中，使用取消默认事件，可以设文本不能被选中
+ 针对图片拖拽时产生禁止拖拽图标，使用取消默认事件，可以取消禁止的图标产生                                         

**案例    [拖拽](./案例/拖拽.html)    [右键菜单创建元素](./案例/右键菜单创建元素.html)    [节流](./案例/节流.html)**
...
2. 严格模式下的限制
   ...
   + 不能使用arguments.callee、arguments.callee.caller--->不想让开发人员使用匿名函数
   + 禁止this指向全局对象，顶层的this指向undefined，不能指向window，即不能在顶层代码中使用this

### const

1. 常量不能修改 const EVENT_TD（不能这样，开辟空间定义了常量，但是没有值，没办法使用）

2. 前端一定要考虑垃圾回收，尤其是页面比较庞大的时候，考虑创建的变量最后能否被清除，不要创建不能被清除的东西

3. const obj = {a:1,b:2}这样做可以防止obj的引用地址发生改变，obj.a=10还是可以的，可以改值，但是引用地址不变

   缺陷：obj不能设置为null，一直存在于堆中，无法进行销毁

### 箭头函数

1. 箭头函数相当于省略了function
2. 如果函数中只有一句话，并且这句话是return，就可以省略function和return
3. 箭头函数中如果只有一个参数，可以省略小括号，如果没有参数或者有多个参数，不能省略小括号
4. 箭头函数的作用：更改this指向，箭头函数主要可以作为改变函数的this指向问题
5. 当函数被换为箭头函数时，this会被指向当前函数外的this的指向

### 解构赋值

1. 解构赋值的作用：

   + 函数返回的时候，将一个数组返回，接收的时候接收为几个单个的变量，自动解构赋值
   + 函数传参的时候，传进去一个数组，形参为一个变量数组，自动解构赋值给各个变量
   + 解构赋值 赋初值，如果有初值则覆盖
...
3. 对象解构赋值的优点：可以在函数当中用来返回值

   + 使用函数当中，可以赋初值
   + 使用函数当中，参数可以不按照类写，可以跳过一个参数，可以跳位，不需要按照顺序写
   + 可以将加载进来的对象的方法解构进来，形成全局的函数

4. 出现重复的名字的时候就另起一个不一样的名字
...
1. includes()判断字符在字符串中是否存在

2. startsWith("b",1)判断a是不是从头开始

   endsWith()

3. 字符串.padStart(长度,前面补充的字符);

4. 字符串.padEnd(长度,后面补充的字符);

### Symbol

1. symbol的含义是唯一，创建的永远没有相同的
2. 用处
   + 去除魔术字符串，强制要求唯一进入判断条件
   + 给对象添加属性和属性值，不希望这个属性和值被修改，对象的属性只能是字符串或者是symbol类型，形成唯一的键名，这个键名就不会被覆盖和修改

### Set和Map

1. Set数据类型 无重复列表类型，没有下标，不是按照下标存储，但是有序
2. Set的有序不可控，不能使用下标循环遍历
3. 插入速度和删除速度非常快
4. 没有重复元素，有重复就会自动去重，任何的元素存在唯一性，遍历查找速度很快，但是略低于键值对类型
5. 方法：
   + add，添加的元素没有顺序
   + delete，删除元素
   + has，查找元素，返回布尔值，判断这个元素是否在列表中存在
   + clear，清除所有的数据
   + size（属性），没有length，size就是长度
6. 用处：
   + 数据去重
   + 添加的时候避免添加重复元素
7. 要是存入两个NaN？只会存入一个
8. Set遍历：forEach、for-of





1. Map 键值对的数据类型，有长度size，一种有长度的键值对数据结构
2. 既具备数组的长度紧密型，又具备对象的键值对方法
3. 获取、删除、查询、遍历速度很快
4. 方法：
   + set添加元素
   + delete删除元素，通过指定key进行删除元素
   + clear清除所有数据，对象不可以
   + get通过键获取值，不能通过点语法
   + has快速判断一个元素是否在map中，判断的是键，不是值，返回布尔值
   + map.values()获取所有值的列表，可以转化成数组和Set
   + map.keys()获取所有键的列表
5. 遍历：
   + forEach，对象不能使用
   + for-of
   + for-of遍历map.entries(),每个键值对返回一个类数组，0为键，1为值
6. 键可以是symbol类型，但是输出的时候键不会展示，只会展示值，可以获取
7. 键可以是Object，以引用地址存值，对象是将所有的key转换为字符串
8. 任何类型均可以作为键使用



1. WeakSet WeakMap 弱引用类型
2. 强引用类型在删除某一个变量的时候，必须将所有引用该变量的地方都设置为null
3. 弱引用类型，其中一个强引用的类型地址设置为null以后，弱引用就会被自动为null，被清除
4. 弱引用的类型不可以遍历
...
1. AJAX技术核心是XMLHttpRequest对象(简称XHR)
...
2. 两个版本的XHR
   ...
   + level1：
     ...
     1. 受同源策略的限制，不能发送跨域请求
     2. 不能发送二进制文件，只能发送纯文本数据
     3. 在发送和获取数据的过程中，无法实时获取进度信息，只能判断是否完成

   + level2：

     1. 早服务端允许的情况下，可以发送跨域请求
     2. 支持发送和接收二进制数据
...
1. 同步：等待运行结束，继续后续的运行
2. 异步：需要等待一个内容完成后继续执行后面的内容，但是不能将后面的内容写在等待函数外，否则就会同时执行两个，（两个步子同时进行）
3. 图片需要预加载，加载需要时间，所以没有做任何处理的时候，图片的尺寸数据打印出来都是0

### Promise

1. 操作性事件，需要时间，
2. load事件、setTimeout、setInterval、requestAnimationFrame是异步
3. 异步async，意思是，创建一个自己的时间轴
4. defer 所有内容全部加载完成，html标签已经完成渲染
5. Promise.all
6. Promise.race
7. 是否可以连续then？可以，但是resolve的参数只能传送到第一层
8. 代码的执行顺序，在Promise对象的then和catch中，都是异步的，除此之外，都是同步的
9. async函数执行后返回一个Promise对象
10. await只能写在async函数中，只能处理promise对象的异步等待
11. async函数中使用return返回的内容可以通过then来获取
...
1. 一般获取本地计算机权限的会在本地存储

2. cookie只能作为临时存储，当浏览器关闭就会丢失

   + 使用场景
     + 刷新页面时使用
     + 跳转到当前域所在另外页面时可以获取前面cookie存储的数据内容
   + cookie的作用域为当前作用域，有文件夹路径的区分
   + 在当前域中任何文件都可以写cookie，存储的位置都是以这个域作为存储空间的，别的域不能访问
   + 顶部的域无法访问下属域的cookie，但是下属域可以向上访问顶级域的cookie
   + cookie是按照与域当中的文件夹存储的

3. cookie可以设置过期时间

4. cookie的两种存储方式：字符串、直接存储对象

5. 如何删除已经设置了过期失效的cookie？

6. md5加密方式是不可逆的，md5的使用方法是，两边同时使用md5，然后对比两边md5加密的结果

7. cookie和localStorage、sessionStorage的不同

   + cookie会自动携带数据发送给服务器，不管需不需要

     localStorage和sessionStorage不会发送
